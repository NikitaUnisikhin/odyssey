						       ...
						       void **argv)
{
	...
	od_list_foreach(obsolete_rules, i)
	{
		...
		if (strcmp(rule->user_name, obsolete_rule->usr_name) == 0 &&
		    ...
		    od_address_inet_equals(&rule->address_range.addr, &obsolete_rule->address_range.addr) &&
		    od_address_inet_equals(&rule->address_range.mask, &obsolete_rule->address_range.mask) &&
		    rule) {
